...
class Overlay():
  ...
  def drawUI( self ):
    _handle = SpaceView3D.draw_handler_add( self.__renderCircle, (), 'WINDOW', 'POST_PIXEL' )
    self.meshes.append( _handle )
    _handle = SpaceView3D.draw_handler_add( self.__renderRailing, (), 'WINDOW', 'POST_PIXEL' )
    self.meshes.append( _handle )
  ...
  def __makeBox( self, view: Region, color: tuple[ float, float, float, float ] ):
    ...
    left_rail = ( Vector( ( 0.0, 0.0 ) ), Vector( ( mid.x * settings.getWidth(), dimensions.y ) ) )
    right_rail = (
      Vector( ( dimensions.x, 0.0 ) ), Vector( ( dimensions.x - mid.x * settings.getWidth(), dimensions.y ) )
    )
